...
const descriptions = [];
const likes =  Array(53).fill(0);
// not gonna stay (included in the database)
...
server.on("request", async (req, res) => {
    if (req.method === "GET"){
        ...
        if (req.url.startsWith('/public/')) {
            ...
            try {
                const sqlQuery = 'SELECT p.fichier, o.nom FROM photos p JOIN orientations o ON p.orientation = o.id WHERE o.nom = \'portrait\''; //Requête SQL
                ...
                for (let i = 0 ; i < fichiersImage.length ; i++) {
                    ...
                    <a href="../j-aime/${i}"><i class="fa-regular fa-thumbs-up"></i></a>
                    <span class="like_count">${likes[i]}</span>
                    </div>
                    </div>`;
                }
                pageHTML += `
            ...
            try {
                const sqlQuery = 'SELECT p.fichier, p.nom AS photo_nom, o.nom, ph.nom, ph.prenom FROM photos p JOIN orientations o ON p.orientation = o.id JOIN photographes ph ON p.id_photographe = ph.id WHERE o.nom = \'portrait\''; //Requête SQL
                ...
                if( descriptions[imgNumber] != null ){
                    pageHTML += `<p > ${descriptions[imgNumber].toString ()} </p> `;
                } else{
            ...
            console.log("here");
            const imageNumber = req.url.slice(8);
        ...
        if ( req.url === "/image-description") {
            ...
            req.on("end", () => {
                ...
                descriptions[imageNumber-1] = decodeURIComponent(description.replace(/\+/g, ' '));
